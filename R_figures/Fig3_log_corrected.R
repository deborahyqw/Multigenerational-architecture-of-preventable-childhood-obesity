...
##############################################################################################
fig3_par_df <- read.csv("Fig3_PAR.csv", check.names = FALSE, header = TRUE)[1:22,] %>%
...
fig3_df <- read.csv("Fig3_heat_forest_pie.csv", check.names = FALSE, header = TRUE)[1:37,1:10] 
...
###################################################################################
# Extract the Cat you want to plot
cat_name <- "Socioeconomic"
cat_color <- group_colors[cat_name]
# Subset the data
plot_data_socio <- df_wide[df_wide$Cat == cat_name, ]

socio_levels <- fig3_par_df %>%
  filter(category == "Socioeconomic", !grepl("Combined", Label)) %>%
  arrange(PAR) %>%
  pull(Label) %>%
  as.character()
...
ggsave("newFig2_logfixed.pdf", plot=fig3_rr, family = "Helvetica", width = 7.3, height = 10)
ggsave("newFig3.pdf", plot=fig3_par, family = "Helvetica", width = 5, height = 6)


CairoPDF("Fig3_logfixed.pdf", family = "Helvetica", width = 10.83, height = 10)
