...
%LET covar1= chbmibase cohort white sex; /*MANUAL SPECIFICATION OF VARIABLES*/
	...
	**********************************************************************/
	...
	_type_ = 'PARM';
		...
		* 'i' counts rows within each _imputation_ group ;
		...
		when (16) white    = estimate;
		when (17) sex     = estimate;
	...
	data covparms;  
		...
		rename  Prm1=intercept   Prm2=chwestq1   Prm3=chwestq2   Prm4=chwestq3   Prm5=chstq1  
				...
				Prm15=cohort     Prm16=white     Prm17=sex ;
...
%ob_par_MI_strat(data=data_child, group=child); 
%ob_par_MI_strat(data=data_teen, group=teen); 
