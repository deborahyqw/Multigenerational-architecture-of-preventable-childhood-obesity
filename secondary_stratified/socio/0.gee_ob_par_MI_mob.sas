...
stratified by maternal obesity status
Exposures: 	&heduc_ [categorical]; &fincome_ [categorical]; sesq_ [quantiles]
Stratifiers: maternal obesity status
...
%LET covar1= midwest south west cohort chage white sex; /*MANUAL SPECIFICATION OF VARIABLES*/
	...
	* Only OUTPUT once per _imputation_, after we've assigned all param columns ;
	...
	data covparms;  
		...
		* Output EVERY row;
		...
		keep _imputation_ _type_ _name_ prm1-prm13 ;  /*MANUAL SPECIFICATION OF PARAMETER RANGE*/
	run;
	...
	* Only OUTPUT once per _imputation_, after we've assigned all param columns ;
	...
	data covparms;  
		...
		* Output EVERY row;
		...
		keep _imputation_ _type_ _name_ prm1-prm16 ; /*MANUAL SPECIFICATION OF PARAMETER RANGE*/
	run;
