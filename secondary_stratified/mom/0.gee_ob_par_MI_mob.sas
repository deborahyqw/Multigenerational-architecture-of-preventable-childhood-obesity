...
%LET covar1= moage cohort chage white sex ; run; /*MANUAL SPECIFICATION OF VARIABLES*/
...
%macro ob_par_MI_strat (data=, group=);
	...
	**********************************************************************/
	...
	title "HEADER: maternal modifiable factors for &group.";
	...
	TITLE "HEADER: multiple imputation - maternal modifiable factors for &group";
	...
	*proc print data=freqs1 (obs=10); run;
	...
	%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=mom,
			modmod = mowestq1  mowestq2 mowestq3   mopaq0    mopaq1    mopaq2   mosmk2 mosmk3  
					moshiftq1 moshiftq2 moshiftq3 ,
			fixfix = mocalq1  mocalq2  mocalq3  &covar1); /*note that to stratify by mob, I did not fix for moob*/
