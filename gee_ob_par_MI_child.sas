/*******************************************************************************
Program name: gee_ob_par_MI_child.sas
Purpose: Calculate PAR for personal exposures for obesity risk with multiple imputation
...
Estimate         0.168516       0.056324     0.058061     0.278971   2183.6       0.159201       0.185679              0               2.99     0.0028
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=west1,
        ...
        fixfix =  chwestq2 chwestq3 chstq1 chstq2 chstq3 
			...
			chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.019230       0.022519     -0.02494     0.063397   1757.3       0.012764       0.024278              0               0.85     0.3933
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=west2,
        ...
        fixfix = chwestq1  chwestq3 chstq1 chstq2 chstq3 
			...
			chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.062823       0.023467     0.016801     0.108844   2117.8       0.057821       0.067802              0               2.68     0.0075
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=west3,
        ...
        fixfix = chwestq1 chwestq2  chstq1 chstq2 chstq3 
			chpaq0 chpaq1 chpaq2 chcalq1 chcalq2 chcalq3 
			 chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.086463       0.025275     0.036801     0.136126   483.93       0.078277       0.097257              0               3.42     0.0007
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=sed,
        ...
        fixfix = chwestq1 chwestq2 chwestq3 
			chpaq0 chpaq1 chpaq2 chcalq1 chcalq2 chcalq3 
			 chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.145726       0.067418     0.013037     0.278416   290.99       0.124113       0.179569              0               2.16     0.0315
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=sed1,
        ...
        fixfix = chwestq1 chwestq2 chwestq3 chstq2 chstq3 
			chpaq0 chpaq1 chpaq2 chcalq1 chcalq2 chcalq3 
			 chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.046651       0.023283     0.000788     0.092515   241.57       0.038892       0.057334              0               2.00     0.0462
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=sed2,
        ...
        fixfix = chwestq1 chwestq2 chwestq3 chstq1  chstq3 
			chpaq0 chpaq1 chpaq2 chcalq1 chcalq2 chcalq3 
			 chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.062148       0.027767     0.007310     0.116987   159.82       0.051113       0.076665              0               2.24     0.0266
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=sed3,
        ...
        fixfix = chwestq1 chwestq2 chwestq3 chstq1 chstq2 
			chpaq0 chpaq1 chpaq2 chcalq1 chcalq2 chcalq3 
			 chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.036927       0.029816     -0.02160     0.095450   832.91       0.030632       0.045571              0               1.24     0.2159
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=pa,
        ...
        fixfix = chwestq1 chwestq2 chwestq3 chstq1 chstq2 chstq3 
			 ...
			 chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.147069       0.050456     0.048150     0.245987   4514.3       0.135652       0.157890              0               2.91     0.0036
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=pa0,
        ...
        fixfix = chwestq1 chwestq2 chwestq3 chstq1 chstq2 chstq3 
			 ...
			 chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.096952       0.022527     0.052800     0.141105    69132       0.094085       0.098973              0               4.30     <.0001
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=pa1,
        ...
        fixfix = chwestq1 chwestq2 chwestq3 chstq1 chstq2 chstq3 
			chpaq0  chpaq2 chcalq1 chcalq2 chcalq3 
			 chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.034118       0.021544     -0.00814     0.076374     1652       0.030060       0.039253              0               1.58     0.1135
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=pa2,
        ...
        fixfix = chwestq1 chwestq2 chwestq3 chstq1 chstq2 chstq3 
			chpaq0 chpaq1  chcalq1 chcalq2 chcalq3 
			 chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.015999       0.021005     -0.02523     0.057223   911.68       0.010097       0.022170              0               0.76     0.4465
;
...
%runPAR(prefixBeta=betacov, prefixFreq=freqs, outpref=child,
        ...
        fixfix =  chcalq1 chcalq2 chcalq3 
			 chbmibase cohort chage white sex);
*Parameter        Estimate      Std Error    95% Confidence Limits        DF        Minimum        Maximum         Theta0   Parameter=Theta0   Pr > |t|
Estimate         0.425044       0.079707     0.268560     0.581528   727.05       0.393502       0.445822              0               5.33     <.0001
;
