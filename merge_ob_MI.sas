...
1) Purpose: Combine multiple imputed datasets and transform wide to long
2) Study Population: 
...
*Follow-up;
  	...
  	*%exclude(0 lt lastq lt irt{time}); 	 *censor lost to follow up 0 - not needed for GEE ;
	...
	%exclude(momid ne ., nodelete=t); *383089;
...
*Follow-up;
  	...
  	*%exclude(0 lt lastq lt irt{time}); 	 *censor lost to follow up 0 ;
...
proc freq data=all; 
	table gotweight &abwt_ &gweek_ &prev_preg_ &fincome_ incom01 income
		  Comp_Gesdbg Comp_prghtng Comp_peclmpg Delivery pregcomp pregcomp2
		  husbeduc &mosmk_ chob moob bpregob;
run;
*/