...
data all; set all end=_end_;
  ...
  /*
    retain baselineYear . removeID 0;  
    if first.ID then do;
       baselineYear = .;  
       removeID = 0;      
    end;  	 	
    if missing(baselineYear) and not missing(gotweight) then baselineYear = year;
    if missing(baselineYear) then delete;
    if year = baselineYear then do;
       if chob=1 or chbmi=. or lastq=chirt then removeID=1;
    end;
    if removeID=1 then delete;
    if year > baselineYear and missing(gotweight) then delete;
    */
run; 
...
Estimate         0.022237       0.013590     -0.00440     0.048874    14719       0.019860       0.023820              0               1.64     0.1018
;
